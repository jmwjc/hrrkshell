...
\end{equation}
where $\boldsymbol f_I$'s \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{denote }\DIFaddend the components of the traditional force vector:
\begin{equation}
...
\end{split}
\end{equation}
\DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{The complete discrete equilibrium equations can be obtained by }\DIFaddend further substituting Eqs. (\ref{epsilon2}) and (\ref{kappa2}) into above equation\DIFdelbegin \DIFdel{gives the final discrete equilibrium equations}\DIFdelend , respectively: 
\begin{equation}
        (\boldsymbol K + \tilde{\boldsymbol K} + \bar{\boldsymbol K} )\boldsymbol d = \boldsymbol f + \tilde{\boldsymbol f} + \bar{\boldsymbol f}
\end{equation}
where \DIFaddbegin \DIFadd{the components of stiffness matrices and force vectors in discrete equilibrium equations can be evaluated as follows:
}\DIFaddend \begin{equation}\label{de1}
...
The detailed derivations of Eqs (\ref{de1})-(\ref{de3}) are listed in the \ref{derivations}. As shown in these equations, Eq. (\ref{de1}) is the conventional stiffness matrix evaluated by smoothed gradients $\tilde \Psi_{I,\alpha}$, $\tilde \Psi_{I,\alpha}\vert_\beta$, and the Eqs. (\ref{de2}) and (\ref{de3}) contribute for the enforcement of essential boundary. It should be \DIFdelbegin \DIFdel{mentioned }\DIFdelend \DIFaddbegin \DIFadd{noticed }\DIFaddend that, in accordance with reproducing kernel smoothed gradient framework, the integration scheme of Eqs. (\ref{de1}-\ref{de3}) should be aligned with \DIFdelbegin \DIFdel{the }\DIFdelend those used in the construction of smoothed gradients. The integration scheme used for \DIFaddbegin \DIFadd{the }\DIFaddend proposed method is shown in Fig. \ref{fig2}, \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{in which the total number of the blue circular integration points has been optimized from a global point of view, aiming to reduce the computation of traditional meshfree shape functions and its first order derivatives. In contrast, for assembly stiffness matrix $\boldsymbol K$, the low order Gauss integration rule is suitable to ensure the accuracy due to the inherently variational consistency in the smoothed gradients. The }\DIFaddend detailed positions and weight of \DIFdelbegin \DIFdel{integration points }\DIFdelend \DIFaddbegin \DIFadd{the integration points and the efficiency demonstration of this optimized integration scheme }\DIFaddend can be found in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{du2022}  }\hskip0pt%DIFAUXCMD
With a close look at }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{wang2019a, du2022}}\hskip0pt%DIFAUXCMD
.  Examining }\DIFaddend Eqs. (\ref{de2}) and (\ref{de3}), \DIFdelbegin \DIFdel{the proposed approach for enforcing }\DIFdelend \DIFaddbegin \DIFadd{closely reveal that the structure of the suggested approach to enforce }\DIFaddend essential boundary conditions \DIFdelbegin \DIFdel{show an identical structure with traditional }\DIFdelend \DIFaddbegin \DIFadd{is identical to that of the conventional }\DIFaddend Nitsche's method, \DIFdelbegin \DIFdel{both have }\DIFdelend \DIFaddbegin \DIFadd{with both having }\DIFaddend the consistent and stabilized terms. \DIFdelbegin \DIFdel{So, the next subsection will review the }\DIFdelend \DIFaddbegin \DIFadd{Thus, a review of }\DIFaddend Nitsche's method and \DIFdelbegin \DIFdel{compare it }\DIFdelend \DIFaddbegin \DIFadd{a comparison }\DIFaddend with the proposed \DIFdelbegin \DIFdel{method. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{approach will be provided in the next subsection. 
}\DIFaddend \subsection{Comparison with Nitsche's method}
...
\int_\Omega \frac{1}{2} \kappa_{\alpha\beta}M^{\alpha\beta} d\Omega \\
                     ...
                     &+ \DIFdelbegin %DIFDELCMD < \underbrace{\frac{\alpha_v}{2} \int_{\Gamma_v} \boldsymbol v \cdot \boldsymbol v d\Gamma 
%DIFDELCMD <                      + \frac{\alpha_\theta}{2} \int_{\Gamma_\theta} \theta_{\boldsymbol n}^2 d\Gamma
%DIFDELCMD <              + \frac{\alpha_C}{2}(\boldsymbol v \cdot \boldsymbol v)_{\boldsymbol x\in C_v}}%%%
...
\end{split}
\end{equation}
where the consistent term generated from the Lagrangian multiplier method contributes to enforce the essential boundary, and meet the variational consistency condition. However, the consistent term can not always ensure the coercivity of stiffness, so the penalty method is introduced to serve as a stabilized term\DIFaddbegin \DIFadd{, in which $\alpha_{vi}$ is the experimental artificial parameter for enforcing the displacement towards $\boldsymbol a_i$ direction, $\alpha_\theta$ and $\alpha_C$ are parameters for enforcing rotation and corner deflection}\DIFaddend . With a standard variational argument, the corresponding weak form can be stated as:
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
\DIFdelbegin \DIFdel{in which $\alpha_v$, $\alpha_\theta$ and $\alpha_C$ represent experimental artificial parameters. Further }\DIFdelend \DIFaddbegin \DIFadd{Upon further }\DIFaddend invoking the conventional reproducing kernel approximation of Eq. (\ref{approxv})\DIFdelbegin \DIFdel{leads to the following }\DIFdelend \DIFaddbegin \DIFadd{, the subsequent }\DIFaddend discrete equilibrium equations \DIFdelbegin \DIFdel{:
}\DIFdelend \DIFaddbegin \DIFadd{can be obtained:
...
\end{equation}
where the stiffness \DIFdelbegin \DIFdel{$\boldsymbol K_{IJ}$ }\DIFdelend \DIFaddbegin \DIFadd{$\boldsymbol K$ }\DIFaddend is identical with Eq. (\ref{de1}). \DIFdelbegin \DIFdel{$\boldsymbol K^c_{IJ}$ and $\boldsymbol K^s_{IJ}$ }\DIFdelend \DIFaddbegin \DIFadd{$\boldsymbol K^c$ and $\boldsymbol K^s$ }\DIFaddend are the stiffness matrices for consistent and stabilized terms, respectively, and \DIFaddbegin \DIFadd{their components }\DIFaddend have the following form:
...
\begin{align}
\boldsymbol K^s_{IJ} &= \DIFaddbegin \boldsymbol \DIFaddend \alpha_v \int_{\Gamma_v} \Psi_I \Psi_J \DIFdelbegin %DIFDELCMD < \boldsymbol %%%
...
On comparing with the consistent terms of Eqs. (\ref{de2}) and (\ref{nde2}), the expressions were almost identical, the major difference is that the higher order derivatives of shape functions have been replaced by \DIFaddbegin \DIFadd{the }\DIFaddend smoothed gradients. Owing to the reproducing kernel framework, the construction of \DIFaddbegin \DIFadd{the }\DIFaddend smoothed gradients only concerned about the computation of traditional meshfree shape functions and their first order derivatives, which avoid the costly computation of higher order derivatives. Moreover, the stabilized terms in Eq. (\ref{nde3}) employs the penalty method \DIFaddbegin \DIFadd{with big enough artificial parameters }\DIFaddend to ensure the coercivity of stiffness\DIFaddbegin \DIFadd{. Besides, the optimal values of these artificial parameters are proportional to the grid size of discrete model that can be represented by the support size in meshfree approximation, where $\alpha_{v\alpha} \propto s^{-1}$, $\alpha_{v3} \propto s^{-3}$, $\alpha_\theta \propto s^{-1}$, $\alpha_C \propto s^{-2}$\mbox{%DIFAUXCMD
