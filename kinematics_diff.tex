...
\end{equation}
where $\boldsymbol a^\alpha$'s is the in-plane contravariant base vector, $\boldsymbol a^\alpha \cdot \boldsymbol a_\beta = \delta^\alpha_\beta$, $\delta$ is the Kronecker delta function. \DIFdelbegin \DIFdel{Substituting }\DIFdelend \DIFaddbegin \DIFadd{The detailed derivation of Eq. \ref{a3} can be found in reference \mbox{%DIFAUXCMD
\cite{benzaken2021}}\hskip0pt%DIFAUXCMD
.
}

\DIFadd{Furthermore, substituting }\DIFaddend Eq. (\ref{a3}) into Eq. (\ref{kappa1}) leads to:
\begin{equation}
